...
function VeteranBadge({ rewardKeys, style }: { rewardKeys?: Set<string>; style?: React.CSSProperties }) {
  ...
  return (
    <div style={{ border: '1px solid rgba(201,168,76,0.5)', borderRadius: '2px', padding: '5px 10px', fontFamily: FONT.body, fontSize: '9px', letterSpacing: '0.4em', textTransform: 'uppercase' as const, color: 'rgba(201,168,76,0.7)', ...style }}>
      Veteran
    </div>
  )
}
  ...
  return (
    <div style={{ ...VARIANT_INNER, alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ paddingTop: '72px', display: 'flex', flexDirection: 'column', alignItems: 'center', ...Z2 }}>
        <span style={{ fontFamily: FONT.body, fontSize: '14px', color: '#C9A84C', letterSpacing: '0.35em', textTransform: 'uppercase', fontWeight: '600' }}>MISSION</span>
  ...
  return (
    <div style={{ ...VARIANT_INNER }}>
      ...
      <div style={{ padding: '72px 80px 0', ...Z2 }}>
        <p style={{ fontFamily: FONT.body, fontSize: '13px', color: '#C9A84C', letterSpacing: '0.4em', textTransform: 'uppercase', fontWeight: '600' }}>MISSION</p>
      </div>
      {/* Bottom content */}
    ...
    <div style={{ ...VARIANT_INNER, alignItems: 'center' }}>
      ...
      <div style={{ paddingTop: '80px', ...Z2 }}>
        <div style={{ width: '160px', height: '160px', borderRadius: '50%', border: '2px solid rgba(201,168,76,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '4px' }}>
          <span style={{ fontFamily: FONT.display, fontSize: '48px', fontWeight: '700', color: '#C9A84C', lineHeight: '1' }}>{entry.city?.slice(0, 3).toUpperCase()}</span>
          <span style={{ fontFamily: FONT.body, fontSize: '10px', color: '#8C8680', letterSpacing: '0.3em', textTransform: 'uppercase' }}>{entry.country_code || entry.country?.slice(0, 2).toUpperCase()}</span>
        </div>
      </div>
      {/* Bottom content */}
  ...
  return (
    <div style={{ ...VARIANT_INNER, alignItems: 'center' }}>
      ...
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 1, opacity: 0.06 }}>
        <span style={{ fontFamily: FONT.display, fontSize: '240px', fontWeight: '700', color: '#C9A84C', whiteSpace: 'nowrap', letterSpacing: '-0.04em' }}>{entry.city}</span>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '80px', gap: '24px', ...Z2 }}>
        <div style={{ border: '1px solid rgba(201,168,76,0.3)', borderRadius: '4px', padding: '6px 16px' }}>
          <span style={{ fontFamily: FONT.body, fontSize: '11px', letterSpacing: '0.35em', textTransform: 'uppercase', color: '#C9A84C', fontWeight: '600' }}>MISSION</span>
        </div>
