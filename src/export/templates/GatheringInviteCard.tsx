...
const GatheringInviteCard = React.forwardRef<HTMLDivElement, Props>(({ entry, backgroundUrl, rewardKeys }, ref) => {
  ...
  return (
    <div ref={ref} style={{ width: '1080px', height: '1350px', backgroundColor: '#0D0B0F', fontFamily: 'var(--font-body)', overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '80px', paddingLeft: '80px', paddingRight: '80px', width: '100%', position: 'relative', zIndex: 2 }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '72px', color: '#F0EDE8', textAlign: 'center', lineHeight: 1.15, margin: 0, fontWeight: 400 }}>{entry.title}</h1>
      </div>
      {/* Bottom: details */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', paddingBottom: '80px', paddingLeft: '80px', paddingRight: '80px', position: 'relative', zIndex: 2 }}>
        ...
        <p style={{ fontFamily: 'var(--font-body)', fontSize: '22px', color: backgroundUrl ? '#A09890' : '#8C8680', textAlign: 'center', marginBottom: '8px' }}>{formatDate(eventDate)}</p>
        ...
        {entry.description && <p style={{ fontFamily: 'var(--font-display)', fontSize: '20px', color: backgroundUrl ? '#C8C0B0' : '#8C8680', textAlign: 'center', fontStyle: 'italic', maxWidth: '760px', lineHeight: 1.6, marginBottom: '48px' }}>{entry.description}</p>}
