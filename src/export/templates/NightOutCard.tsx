  ...
  return (
    <div style={{ ...VARIANT_INNER, alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '56px', paddingLeft: '80px', paddingRight: '80px', width: '100%', ...Z2 }}>
        ...
        <h1 style={{ fontFamily: FONT.display, fontSize: '80px', fontWeight: '700', color: '#F0EDE8', textAlign: 'center', lineHeight: '1.05', letterSpacing: '-0.02em', margin: '0 0 36px 0' }}>{entry.title}</h1>
        {entry.location && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '32px' }}>
            <div style={{ width: '24px', height: '1px', backgroundColor: '#C9A84C', opacity: 0.6 }} />
            <span style={{ fontFamily: FONT.body, fontSize: '24px', color: '#C9A84C', letterSpacing: '0.08em' }}>{entry.location}</span>
            <div style={{ width: '24px', height: '1px', backgroundColor: '#C9A84C', opacity: 0.6 }} />
          </div>
        )}
        <p style={{ fontFamily: FONT.body, fontSize: '16px', color: '#8C8680', letterSpacing: '0.18em', textTransform: 'uppercase', margin: 0 }}>{formatDate(entry.date)}</p>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', paddingLeft: '80px', paddingRight: '80px', ...Z2 }}>
        ...
        {oneliner && (
          <p style={{ fontFamily: FONT.display, fontStyle: 'italic', fontSize: '26px', color: backgroundUrl ? '#C8C0B0' : '#8C8680', textAlign: 'center', lineHeight: '1.6', maxWidth: '860px' }}>{oneliner}</p>
        )}
      </div>
  ...
  return (
    <div style={{ ...VARIANT_INNER }}>
      ...
      <div style={{ padding: '72px 80px 0', ...Z2 }}>
        <p style={{ fontFamily: FONT.body, fontSize: '13px', color: '#C9A84C', letterSpacing: '0.4em', textTransform: 'uppercase', fontWeight: '600' }}>NIGHT OUT</p>
      </div>
      {/* Bottom content */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '0 80px', ...Z2 }}>
        <h1 style={{ fontFamily: FONT.display, fontSize: '96px', fontWeight: '700', color: '#F0EDE8', lineHeight: '0.95', letterSpacing: '-0.03em', margin: '0 0 32px 0' }}>{entry.title}</h1>
        {oneliner && (
          <p style={{ fontFamily: FONT.display, fontStyle: 'italic', fontSize: '24px', color: '#C8C0B0', lineHeight: '1.5', maxWidth: '800px', marginBottom: '40px' }}>{oneliner}</p>
        )}
    ...
    <div style={{ ...VARIANT_INNER, alignItems: 'center' }}>
      ...
      {oneliner && (
        <div style={{ paddingTop: '80px', paddingLeft: '80px', paddingRight: '80px', ...Z2 }}>
          <p style={{ fontFamily: FONT.display, fontStyle: 'italic', fontSize: '40px', fontWeight: '400', color: '#F0EDE8', textAlign: 'center', lineHeight: '1.45', maxWidth: '800px' }}>"{oneliner}"</p>
        </div>
      )}
      {/* Bottom: attribution */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '0 80px 80px', gap: '48px', ...Z2 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
            <span style={{ fontFamily: FONT.display, fontSize: '20px', color: '#C9A84C', fontWeight: '600' }}>{entry.title}</span>
            <span style={{ fontFamily: FONT.body, fontSize: '13px', color: '#8C8680', letterSpacing: '0.15em', textTransform: 'uppercase' }}>{formatDate(entry.date)}</span>
          </div>
          <div style={{ height: '1px', width: '60px', backgroundColor: '#C9A84C', opacity: 0.5 }} />
        </div>
        <BrandMark size="sm" />
    ...
    <div style={{ ...VARIANT_INNER }}>
      ...
      <div style={{ padding: '72px 80px 0', ...Z2 }}>
        <div style={{ border: '1px solid rgba(201,168,76,0.4)', borderRadius: '4px', padding: '6px 16px', display: 'inline-block' }}>
          <span style={{ fontFamily: FONT.body, fontSize: '11px', letterSpacing: '0.35em', textTransform: 'uppercase', color: '#C9A84C', fontWeight: '600' }}>NIGHT OUT</span>
        </div>
      </div>
      {/* Content — bottom aligned */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '0 80px', ...Z2 }}>
        <p style={{ fontFamily: FONT.display, fontSize: '120px', fontWeight: '700', color: 'rgba(201,168,76,0.15)', lineHeight: '1', letterSpacing: '-0.04em', marginBottom: '24px' }}>
          {new Date(entry.date + 'T00:00:00').toLocaleDateString('en-GB', { day: '2-digit', month: 'short' }).toUpperCase()}
        </p>
        <h1 style={{ fontFamily: FONT.display, fontSize: '64px', fontWeight: '700', color: '#F0EDE8', textAlign: 'center', lineHeight: '1.05', margin: '0 0 24px 0' }}>{entry.title}</h1>
        {entry.location && (
          <span style={{ fontFamily: FONT.body, fontSize: '18px', color: '#C9A84C', letterSpacing: '0.1em', marginBottom: '32px' }}>{entry.location}</span>
        )}
        {oneliner && (
          <p style={{ fontFamily: FONT.display, fontStyle: 'italic', fontSize: '22px', color: backgroundUrl ? '#C8C0B0' : '#8C8680', textAlign: 'center', lineHeight: '1.6', maxWidth: '780px' }}>{oneliner}</p>
        )}
      </div>
