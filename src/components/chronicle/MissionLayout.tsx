...
interface MissionLayoutProps {
  ...
  /** Slot for LoreSection (director's notes + generate) rendered by parent */
  loreSlot?: React.ReactNode
  /** Slot for reactions, comments, actions rendered by parent */
...
export function MissionLayout({ entry, photos, isCreator, onEntryUpdate, loreSlot, controlsSlot }: MissionLayoutProps) {
  ...
  return (
    <div className="px-4 py-5 space-y-0">
      ...
      {/* ═══════════════════════════════════════════
          MAGAZINE STORY — Lore + interspersed photos
          ═══════════════════════════════════════════ */}
      {(entry.lore || photos.length > 0) && (
        <>
          ...
          {/* Lore with interspersed photos */}
          {paragraphs.length > 0 && paragraphs.map((p, i) => (
            <div key={i}>
              <p
                className={cn(
                  'font-display text-[15px] text-ivory/85 leading-[1.7] mb-5',
                  i === 0 && 'first-letter:text-[48px] first-letter:float-left first-letter:leading-[1] first-letter:mr-2 first-letter:text-gold first-letter:font-bold',
                )}
              >
                {p}
              </p>
      ...
      {/* ═══════════════════════════════════════════
          DIRECTOR'S NOTES (lore generation controls)
          ═══════════════════════════════════════════ */}
      {loreSlot && (
        <div className="mt-2">
          {loreSlot}
      ...
      {/* ═══════════════════════════════════════════
          CONTROLS — Reactions, comments, actions (expandable)
          ═══════════════════════════════════════════ */}
      {controlsSlot && (
        <>
          <div className="mt-6">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 text-xs text-ivory-dim/60 font-body tracking-wide hover:text-ivory-dim transition-colors"
            >
