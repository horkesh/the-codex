 ...
 * Deduplicates points within 50m to avoid redundant API calls.
 * Returns a Map from "lat.toFixed(6),lng.toFixed(6)" key to location info.
 */
export async function batchReverseGeocode(
  points: { lat: number; lng: number }[]
): Promise<Map<string, { city: string; country: string; country_code: string; venue: string | null }>> {
  ...
  // Deduplicate: cluster points within 50m
  ...
  for (const point of points) {
    ...
    let found = false
    for (const cluster of clusters) {
      const dist = haversineMetres(point.lat, point.lng, cluster.lat, cluster.lng)
  ...
  // Geocode each unique cluster with small delay between calls
  for (const cluster of clusters) {
    try {
      const geo = await reverseGeocode(cluster.lat, cluster.lng)
      const result = {
      ...
      // Small delay between calls to avoid API quota
